...
# Start tigervnc server directly (this includes X server)
echo "Starting tigervnc server on display :1..." >> $LOG_FILE
...
if pgrep -f "Xtigervnc :1" > /dev/null; then
    echo "tigervnc started successfully on :1" >> $LOG_FILE
else
